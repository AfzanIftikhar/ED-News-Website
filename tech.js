...
async function heroNews(){
    let API_URL =  `https://newsapi.org/v2/everything?q=technology&language=en&sortBy=publishedAt&pageSize=1&apiKey=${API_Key}`;
...
let dropDown = document.getElementById("companyFilter");

async function loadTopheadlines(company) {

    let cards_Container = document.querySelector(".news-cards")
    cards_Container.innerHTML = "";
    const API_URL = `https://newsapi.org/v2/everything?q=${company}&language=en&sortBy=publishedAt&pageSize=5&apiKey=${API_Key}`;;
    await fetch(API_URL)
        .then(res => res.json())
        .then(data => {
...
async function latestNews(company){
    ...
    let API_URL = `https://newsapi.org/v2/everything?q=${company}&language=en&sortBy=publishedAt&pageSize=10&apiKey=${API_Key}`
